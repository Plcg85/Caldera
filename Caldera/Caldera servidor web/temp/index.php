<?php
	...
	$mysqli = new mysqli("localhost", "root", "jj2hkr7g", "Temperaturas");
	...
	function getLastTemps($mysqli) {
		$stmt = $mysqli->prepare("SELECT temp1, temp2, temp3, temp4, created_at FROM temps ORDER BY created_at DESC LIMIT 1");
		$stmt->execute();
		$stmt->bind_result($res['temp1'], $res['temp2'], $res['temp3'], $res['temp4'], $res['created_at']);
		$stmt->fetch();
		return $res;
	}
	
	function getAVGTemps($mysqli) {
		$stmt = $mysqli->prepare("SELECT AVG(temp1), AVG(temp2), AVG(temp3), AVG(temp4) FROM temps WHERE created_at >= SYSDATE() - INTERVAL 1 DAY");
		$stmt->execute();
		$stmt->bind_result($res['temp1'], $res['temp2'],$res['temp3'], $res['temp4']);
		$stmt->fetch();
		return $res;
	}

	function last24hourValues($mysqli) {
		$stmt = $mysqli->prepare("SELECT temp1, temp2, temp3, temp4, created_at FROM temps WHERE created_at >= SYSDATE() - INTERVAL 1 DAY");
		$stmt->execute();
		$stmt->bind_result($res['temp1'], $res['temp2'], $res['temp3'], $res['temp4'], $res['created_at']);
		$rows = array();
	...
	function tempParts($temp, $index) {
		$parts = explode('.', number_format($temp, 2));
		return $parts[$index];
	}	
...
<html>
	<head>
		...
		temp3: <?php print number_format($row['temp3'], 2); ?>,
	...
	<body>
		<div class="content">
			<div class="thermometers">
				<div class="label">Temp Casa--rojo</div><div class="label">Temp Seleccionada--negro</div><div class="label">Temp Caldera--azul</div><div class="label">Bomba--morado</div>
				<div class="de">
					<div class="den">
					  <div class="dene">
						<div class="denem">
				...
				<div class="details">Media 24h<br /><?php print number_format($lastAVGTemps['temp1'], 1); ?>&deg;</div><div class="details">Media 24h<br /><?php print number_format($lastAVGTemps['temp2'], 1); ?>&deg;</div><div class="details">Media 24h<br /><?php print number_format($lastAVGTemps['temp3'], 1); ?>&deg;</div><div class="details">Media 24h<br /><?php print number_format($lastAVGTemps['temp4'], 1); ?>&deg;</div>
		...
		<form action="#" method="get">
			<button class="botonBajar" type="submit" formaction="bajarTemperatura.php" value="Bajar 0.5ºC">Bajar 0.5ºC</button>
			<button class="botonSubir" type="submit" formaction="subirTemperatura.php" value="Subir 0.5ºC">Subir 0.5ºC</button>
		</form>
